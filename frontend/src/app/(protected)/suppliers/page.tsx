...
export default function SuppliersPage() {
    ...
    const clients = (clientsData || []).filter((c: any) => c.type === 'Supplier');
        ...
        <div className="space-y-6 pb-12">
            <div className="flex justify-between items-end mb-8">
                <div>
                    ...
                    <p className="text-slate-400 mt-2 text-lg">Manage your integrated system of suppliers and global logistics vendors.</p>
                </div>
                <motion.button
            ...
            >
                {clients.length === 0 ? (
                    <div className="text-center py-12 bg-slate-900/40 rounded-xl border border-white/5">
                        ...
                        <h4 className="text-xl font-medium text-slate-300">No Suppliers Found</h4>
                        <p className="text-slate-500 mt-2">Looks like your supplier directory is empty.</p>
                    </div>
                ) : (
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <form onSubmit={handleCreate} className="space-y-5">
                                <div>
                                    <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Supplier Name</label>
                                    <input type="text" required className="w-full bg-slate-800/50 border border-slate-700 text-white rounded-xl py-3 px-4 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all placeholder:text-slate-600" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Acme Logistics" />
                                </div>
                                <div>
                                ...
                                <div className="mt-8 flex justify-end gap-4 pt-4 border-t border-white/5">
                                    ...
                                    <button type="submit" className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl transition-all shadow-lg shadow-indigo-600/30">Save Supplier</button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
